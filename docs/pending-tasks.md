...
- [ ] **Frank: aplicar migración `add_park_content_columns` y regenerar tipos**:
  Correr `supabase db push` para añadir columnas `nps_description`, `research_status`, `meta_title`, `meta_description`, `last_verified_at` a `public.destinations`, luego regenerar tipos:
  `npx supabase gen types typescript --project-id vrixiuvnhvqafmxlcyex > src/integrations/supabase/types.ts`
  **Hasta que esta migración esté aplicada, `generate-park-content` no encontrará ningún parque pendiente (la columna `research_status` no existirá).**
...
- [2026-06-05] Created Edge Function `generate-park-content` — batch AI content generation for national parks. Queries up to 5 `destinations` rows where `experience_type = 'parque-nacional'` AND `research_status = 'pendiente'`; for each park calls OpenAI Chat Completions (`OPENAI_MODEL`) with `NOMADERIA_SOUL` as system prompt and NPS park data (official_name, nps_description, region, designation, entrance_fee_usd) as user prompt; generates: title (Spanish natural name), short_description, why_visit_markdown, difficulty_description, best_season, season_to_avoid, meta_title (≤60 chars), meta_description (≤155 chars) as a single JSON object; never overwrites non-null/non-empty fields (title exception: overwrites when still matches official_name English placeholder); updates row with patch + `research_status = 'ai_draft'` + `last_verified_at = now()`; returns `{ processed, updated, failed }`. Also: added migration `20260605100000_add_park_content_columns.sql` (adds `nps_description`, `research_status`, `meta_title`, `meta_description`, `last_verified_at` to destinations); updated `ingest-national-parks` to store `nps_description` and explicitly set `research_status = 'pendiente'` on INSERT; added "Generar Contenido AI" button (Wand2 icon) to AdminDashboard quick-actions row. Deployed both functions. TypeScript clean, build passes. **Requires Frank to run `supabase db push` for the migration before use.**
- [2026-06-05] `ingest-national-parks`: supplement territory/edge-case parks — added `fetchParksByCode(apiKey, codes)` helper (single NPS API request via `?parkCode=viis,npsa,jeff`) called after `fetchAllNationalParks`; deduplicates by `parkCode.toLowerCase()` before merging, so parks already captured by the designation filter are never double-counted. Designation filter unchanged. Redeployed. TypeScript clean, build passes.
- [2026-06-05] Sync `NOMADERIA_SOUL` runtime constant with `docs/NOMADERIA_SOUL.md` — updated `supabase/functions/_shared/nomaderia-soul.ts` to match the full `.md` exactly: added "Los datos cambian" 5th Core Truth, expanded Worldview with "Nuestra especialidad" subsection and richer "Dinero y burocracia" bullets (surcharge + America the Beautiful detail), expanded Expertise to full 13-bullet fluency list, added seasonal-data bullet to Boundaries, added `destinations` table reference to Memory Policy, expanded Pet Peeves to separate "Nunca usa" / "Evita" lists. Export structure (`export const NOMADERIA_SOUL = \`...\``) untouched. TypeScript clean, build passes.
- [2026-06-05] Fix `ingest-national-parks` designation filter — changed `=== "National Park"` to `.includes("National Park")` so Alaska parks designated "National Park and Preserve" (e.g. Wrangell-St. Elias, Katmai, Lake Clark, Denali, Gates of the Arctic, Kenai Fjords, Glacier Bay) are now captured. Redeployed to production. TypeScript clean, build passes.
