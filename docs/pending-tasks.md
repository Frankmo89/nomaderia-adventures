...
- [ ] **`DestinationDetail` — cast `DestExt` temporal:** los campos `why_visit_markdown`, `signature_hikes`, `top_activities`, `max_elevation_ft`, `altitude_warning`, `beginner_friendly`, `not_ideal_if` se leen con cast local (`interface DestExt`) hasta que Frank regenere los tipos de Supabase (ver tarea en Pendientes Humanos). Eliminar `DestExt` y los casts tras la regeneración. Nota: `max_elevation_ft` y `altitude_warning` no tienen migración SQL propia — confirmar que las columnas existen en la DB. `beginner_friendly` y `not_ideal_if` sí tienen migración (`20260606110000`).
