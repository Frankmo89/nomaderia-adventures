...
- [ ] **`DestinationDetail` — cast `DestExt` temporal:** los campos `why_visit_markdown`, `signature_hikes` y `top_activities` de la sección "Por qué ir" se leen con cast local hasta que Frank regenere los tipos de Supabase (ver tarea en Pendientes Humanos). Eliminar `interface DestExt` y los casts tras la regeneración.
