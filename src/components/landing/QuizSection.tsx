      ...
      <div className={`relative overflow-hidden ${index === 0 ? "h-64 sm:h-80" : "h-52 sm:h-56"}`}>
        ...
        <div className="absolute top-4 left-4">
          <span className="inline-block px-3 py-1.5 rounded-full text-xs sm:text-sm font-semibold bg-background/90 backdrop-blur-sm text-foreground shadow-md">
            {matchLabels[index] ?? matchLabels[2]}
          </span>
        </div>
...
const QuizResults = ({ results }: { results: QuizDestination[] }) => (
...
const QuizSection = () => {
  ...
  if (showResults) return <QuizResults results={results} />;
            ...
            <motion.div key="email" initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -60 }}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                ...
                <Button onClick={handleSubmit} disabled={loading}
                  className="bg-primary text-primary-foreground whitespace-nowrap shadow-lg shadow-primary/20 h-12 text-base">
