...
const AdminItineraryTemplateEditor = () => {
  ...
  const [content, setContent] = useState<ContentV1>({ version: 1, dias: [] });
  const [loading, setLoading] = useState(true);
  const contentRef = useRef<ContentV1>({ version: 1, dias: [] });
  ...
  const form = useForm<BlockFormValues>({
    ...
    defaultValues: {
      ...
      fuente_url: "",
      afiliado_url: "",
      verify_flag_on: false,
      distancia_km: "",
      desnivel_m: "",
      apto_principiante: false,
      duracion: "",
      modo: "",
    },
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (c.dias.length > 0) setExpandedDay(0);
      setLoading(false);
    })();
  }, [id]);
  ...
  // ── Undo (defined before handleDeleteBlock) ───────────────────────────────────
  ...
  return (
    ...
    >
      {/* Outer container — light bg, bottom padding for footer + FAB */}
            ...
            {reorderMode ? (
              ...
              <>
                ...
                <p className="text-[11px] text-sidebar-foreground/50 mt-0.5">
                  Plantilla · {isPublished ? "publicada" : "borrador"}
                </p>
              </>
            )}
          </div>
            ...
            <Form {...form}>
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                  ...
                  {/* Delete (only when editing existing block) */}
                  {editingBlock && (
