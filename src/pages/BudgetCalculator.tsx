...
import { Calculator, Plane, Hotel, UtensilsCrossed, Compass, Backpack, ShieldCheck, Download, ArrowRight } from "lucide-react";
...
const BudgetCalculator = () => {
  ...
  const breakdown = useMemo(() => {
    ...
    const flights: [number, number] = r.flights;
    const accomLow = r.accommodation[0] * days;
    const accomHigh = r.accommodation[1] * days;
    const foodLow = r.food[0] * days;
    const foodHigh = r.food[1] * days;
    const activities = r.activities;
    const gear = gearEstimate;
    const insurance = insuranceEstimate;

    const totalLow = flights[0] + accomLow + foodLow + activities + gear[0] + insurance[0];
    const totalHigh = flights[1] + accomHigh + foodHigh + activities + gear[1] + insurance[1];

    return {
      items: [
        { label: "Vuelos", icon: Plane, low: flights[0], high: flights[1], color: "hsl(var(--sky))" },
